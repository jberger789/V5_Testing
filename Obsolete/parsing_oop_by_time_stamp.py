...
class Log(object):
	def __init__(self,filename,units):
		...
		self.units = {}
		self.database = sqlite3.connect('test_results.db')
		self.c = self.database.cursor()
		self.c.execute('''CREATE TABLE devices
             (unit INTEGER, side TEXT, test TEXT)''')
	...
	def add_test(self,test):
		...
		for u in self.units:
			for s in SIDES:
				self.c.execute('''INSERT INTO devices (unit, side, test) 
					VALUES (?, ?, ?)''', (u,s,test.tag))
		for u in self.units:
		...
		while (user_in != "go back"):
			...
			if user_in == "options" or user_in == 'o':
				...
				for time_stamp in cur_test_dict[user_in]:
						print(str(str(time_stamp)+": "+str(cur_test_dict[user_in][time_stamp].data_dict)))
